...
function drawComponent(ctx, c, isSelected) {
    ...
    if (c.type === 'NOT_IC') {
        ...
        let valStr = Math.floor(c.val).toString();
        let bands = [parseInt(valStr[0] || 0), parseInt(valStr[1] || 0), Math.max(0, valStr.length - 2)];
        ...
        ctx.fillStyle = c.isBlown ? '#333' : (c.currentI > 0.001 ? `rgba(46, 204, 113, ${Math.min(c.currentI*40, 1)})` : '#555');
        ctx.fill(); ctx.stroke();
    } else if (c.type === 'BAT') {
        ...
        ctx.save(); ctx.translate(isNPN ? c.x+45 : c.x+25, isNPN ? c.y+45 : c.y+38);
